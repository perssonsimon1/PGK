    ...
    def brickInTheWall(x: Int, y: Int): Unit = {
      ...
      for (j <- 1 until blockSize - 1) {
        for (i <- 1 until blockSize - 1) {
          w.moveTo(x * blockSize + xPadding + i, y * blockSize + yPadding + j)
          w.lineTo(x * blockSize + xPadding + i, y * blockSize + yPadding + j)
        }
      }
    }

    for(row <- data.indices) {
      for(col <- data(row).indices) {
        if(data(row)(col)) {
          brickInTheWall(col, row)
        }
      }
    }
