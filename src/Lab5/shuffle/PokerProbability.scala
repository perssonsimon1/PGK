...
object PokerProbability {
  ...
  def test(iterationCount: Int): Map[String, Int] = {
    ...
    var hands: Map[String,Int] = Map(everyHand map { a => a -> 0 }: _*)

    for (i <- 1 to iterationCount) {
      val drawnHand = Hand.drawFrom(cardDeck)
      val res = testHand(drawnHand)
      val count: Int = (hands apply res) + 1

      hands = hands.updated(res, count)
      cardDeck.shuffle()
    }
